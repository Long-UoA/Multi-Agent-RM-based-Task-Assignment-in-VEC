...
%     viscircles([nodesX0,nodesY0],RBS,'LineStyle','--','Color','k')
    ...
    RcpuMax(1,1:nRSU)=20*10^9; % maximum (Hz) in each RSU - 30 GHz
    RcpuMax(1,nRSU+1)=30*10^9; % maximum (Hz) in a BS - 50 GHz
    %--------------------------------------------------------------------------
    RcpuT=ones(1,nRSU+1); % (cycles/s-Hz) assigned to each task
    tempRcpuT=1*10^9;
    for i=1:length(RcpuT)
       if i<length(RcpuT)
           RcpuT(1,i)=randi([1 3])*10^9*RcpuT(1,i); % cycles/s (Hz) at an RSU (from 1 to 3 GHz)
       else
           RcpuT(1,i)=randi([1 3])*10^9*RcpuT(1,i); % cycles/s (Hz) at a BS (from 1 to 3 GHz)
       end
    end
    %--------------------------------------------------------------------------
    for i=1:nVehicle
