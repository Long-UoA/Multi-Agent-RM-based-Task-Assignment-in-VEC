...
for iIter=1:numIter
    ...
    numReq=100; % number of requesting vehicles

    nIteration=3000; % number of time steps
...
%     viscircles([nodesX0,nodesY0],RBS,'LineStyle','--','Color','k')
    ...
    sumPayoffDelay=zeros(N,1);

    sumPayoffCost=zeros(N,1);
...
%        t=1/N;
           ...
           if isempty(VeActIter{1,it}{1,i})
               ...
               if tempRSU==RqsCurrRSU{1,tempVe}(1,2)
                   if RqsCurrRSU{1,tempVe}(1,2)~=0
                       ...
                       uDelay{1,it}{1,i}(1,tempRSU)=-delay;

                       uCost{1,it}{1,i}(1,tempRSU)=-cost;
                       % expected utility
                       for ii=1:S
                          if ii~=tempRSU
                              if ii==S
                              ...
                              uDelay{1,it}{1,i}(1,ii)=-delay;

                              uCost{1,it}{1,i}(1,ii)=-cost;
                          end
                       end
                   elseif RqsCurrRSU{1,tempVe}(1,2)==0
                       ...
                       uDelay{1,it}{1,i}(1,S)=-delay;

                       uCost{1,it}{1,i}(1,S)=-cost;
                       % expected utility
                       for ii=1:S
                          if ii~=S
                              nHop=1;
               ...
               if tempRSU==0
                   ...
                   realPdelay(it,i) = -uDelay{1,it}{1,i}(1,S);

                   realPcost(it,i) = -uCost{1,it}{1,i}(1,S);
               else
                   realP(it,i) = -u{1,it}{1,i}(1,tempRSU);       % Real payoff of user i at iteration it

                   realPdelay(it,i) = -uDelay{1,it}{1,i}(1,tempRSU);

                   realPcost(it,i) = -uCost{1,it}{1,i}(1,tempRSU);
               end
       ...
       cnvThrshld = 0.1; % convergence threshold       
       for i = 1:M
           if isempty(VeActIter{1,it}{1,i})
           else
    ...
    %    fairness(it,1) = (sum(realP(it,1:M),2))^2/sum(xisquare(it,1:M),2)/M;
       ...
       sumPayoffDelay(it,1) = sum(realPdelay(it,1:M),2);

       sumPayoffCost(it,1) = sum(realPcost(it,1:M),2);
       %------------------------Vehicular Mobility-----------------------------
%        if it<=tLmt
%            for iii=1:numReq
%                if posRqs{1,iii}(1,2)==initY+35||posRqs{1,iii}(1,2)==initY+25||posRqs{1,iii}(1,2)==initY+15
...
minDelayCost=min(sumPayoff(1:it,1)')
