...
clc
clear all
% N = 2000;          % Number of interactions                        t = 1..N
...
for iIter=1:numIter
    ...
    nIteration=3000; % number of iterations
...
%     viscircles([nodesX0,nodesY0],RBS,'LineStyle','--','Color','k')
    ...
    for i=1:numReq
        ...
        if i>0&&i<=nL1
            if i==0+1
                posX_target(1,i)=nodesX(1)-1*(D-2*R);
            else
                posX_target(1,i)=posX_target(1,i-1)-tempInterDist;
            end
        ...
        if i>nL3&&i<=nL4
            if i==nL3+1
                posX_target(1,i)=nodesX(10)+1*(D-2*R);
            else
                posX_target(1,i)=posX_target(1,i-1)+tempInterDist;
            end
...
%     nIteration=5000; % number of iterations
...
%     N=nIteration; 
    ...
    tempTcpu=5*10^8; 
    for i=1:length(Tcpu)
       Tcpu(1,i)=tempTcpu*Tcpu(1,i); % CPU cycles (from 0.5 to 1.2 Gcycles)
    end
    %--------------------------------------------------------------------------
    ...
    for i=1:length(RcpuT)
       if i<length(RcpuT)
           RcpuT(1,i)=tempRcpuT*RcpuT(1,i); % cycles/s (Hz) at an RSU (from 1 to 10 GHz)
       else
           RcpuT(1,i)=tempRcpuT*RcpuT(1,i); % cycles/s (Hz) at a BS (from 1 to 20 GHz)
       end
    end
    %--------------------------------------------------------------------------
    for i=1:nVehicle
    ...
    for iii=1:numReq
       if posRqs{1,iii}(1,2)==initY+35||posRqs{1,iii}(1,2)==initY+25||posRqs{1,iii}(1,2)==initY+15
           posRqs{1,iii}(1,1)=posRqs{1,iii}(1,1)+vTarget(1,iii)*t*tLmt;
           ...
           if isempty(VeActIter{1,it}{1,i})
               ...
               if tempRSU==RqsCurrRSU{1,tempVe}(1,2)
                   if RqsCurrRSU{1,tempVe}(1,2)~=0
                       ...
                       tempB=bwV2I;                
                       tempRate=DataRate(tempB,Rx,Ry,Vx,Vy)*t;
...
%                end 
       ...
       %%%%%%%%%%%%%%%%%%%%%%%%%% RLNF %%%%%%%%%%%%%%%%%%%%%%%%%% 
       tempCnt = zeros(1,M); % count until convergence happens
       cnvThrshld = 100; % convergence threshold        
       for i = 1:M
           if isempty(VeActIter{1,it}{1,i})
           else
               ...
               for ii = 1:S
                   if (VeActIter{1,it}{1,i}(1,2)~=0)&&(ii~=VeActIter{1,it}{1,i}(1,2))
                       sumRegret(i,VeActIter{1,it}{1,i}(1,2),ii)=...
                          sumRegret(i,VeActIter{1,it}{1,i}(1,2),ii)...
                          +u{1,it}{1,i}(1,ii)-u{1,it}{1,i}(1,VeActIter{1,it}{1,i}(1,2));
                       %---------------------------------------------------
                       % test convergence rate
                       if (u{1,it}{1,i}(1,ii)-u{1,it}{1,i}(1,VeActIter{1,it}{1,i}(1,2))) <= cnvThrshld 
                       else
                           tempCnt(1,i) = tempCnt(1,i) + 1;
                       end
                       %---------------------------------------------------                       
                       d(ii,it,i)=1/t*sumRegret(i,VeActIter{1,it}{1,i}(1,2),ii);
                   elseif (VeActIter{1,it}{1,i}(1,2)==0)&&(ii~=S)
                       sumRegret(i,S,ii)=...
                          sumRegret(i,S,ii)...
                          +u{1,it}{1,i}(1,ii)-u{1,it}{1,i}(1,S);
                       %---------------------------------------------------
                       % test convergence rate
                       if (u{1,it}{1,i}(1,ii)-u{1,it}{1,i}(1,S)) <= cnvThrshld 
                       else
                           tempCnt(1,i) = tempCnt(1,i) + 1;
                       end
                       %---------------------------------------------------
                       d(S,it,i)=1/t*sumRegret(i,S,ii);
                   end
               end
               for ii=1:S
                  if VeActIter{1,it}{1,i}(1,2)==0
                      probActVehicle{1,i}(1,S)=(1-delta/it^gamma)*max(d(S,it,i),0)/(sum(max(d(:,it,i),0),1)+epsilon)+delta/t^gamma/S;
                  else
                      probActVehicle{1,i}(1,VeActIter{1,it}{1,i}(1,2))=(1-delta/it^gamma)*max(d(VeActIter{1,it}{1,i}(1,2),it,i),0)/(sum(max(d(:,it,i),0),1)+epsilon)+delta/t^gamma/S;
                  end
               end               
               if isempty(VeActIter{1,it}{1,i})
               else
...
minDelayCost=sumPayoff(length(sumPayoff),1)

figure

plot(sumPayoff(1:it,1)')
